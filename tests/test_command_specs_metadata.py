    ...
    for spec in COMMAND_SPECS:
        if "text|json" in spec.usage:
            assert spec.json_output, (
                f"Spec {spec.path} has --format text|json but json_output={spec.json_output}"
            )
...
def test_all_agent_safe_commands_have_json():
    ...
    agent_safe_with_format = [
        ...
        if spec.agent_safe and "text|json" in spec.usage
    ]
    assert len(agent_safe_with_format) > 50, (
        f"Expected at least 50 agent-safe commands with JSON, got {len(agent_safe_with_format)}"
    )
