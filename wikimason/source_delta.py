...
def _index_old_manifest(
    ...
    for key, row in old_manifest.items():
        ...
        if sid:
            old_by_sid[sid] = row
        if path:
            old_by_path[path] = row
        old_by_path[key] = row
    return old_by_path, old_by_sid


def _new_delta_payload() -> dict[str, list[dict[str, Any]]]:
    return {
        "new": [],
        "content_changed": [],
        "metadata_changed": [],
...
def _classify_record_delta(
    record: dict[str, Any],
    prior: dict[str, Any] | None,
    *,
    delta: dict[str, list[dict[str, Any]]],
    actionable_paths: set[str],
) -> None:
    ...
    if prior is not None:
        ...
        if prior_path and prior_path != path and str(prior.get("source_id", "")) == sid:
