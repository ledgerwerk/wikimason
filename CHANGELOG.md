...
- Raised the runtime dependency baseline to ledgercore 0.6.1 with a `<0.7.0` upper bound and aligned WikiMason imports with ledgercore's curated package-root facade.
- Preserved WikiMason's visible vault layout, `wikimason.toml` configuration contract, local front matter renderer, path-containment compatibility, source hashing, manifest semantics, and storage adapters.
- Documented that ledgercore schema-3 storage and migration are not used to manage WikiMason vault content.
